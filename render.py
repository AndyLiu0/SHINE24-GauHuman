...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for _, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        #transforms = torch.zeros_like(transforms)
        #breakpoint()
        # Start timer
        start_time = time.time() 
        render_output = render(view, gaussians, pipeline, background, transforms=None, translation=None)
    ...
    imgs = [(img.cpu().numpy() * 255).astype(np.uint8)[0] for img in rgbs]
    imageio.mimsave(os.path.join(render_path, 'animation.gif'), imgs, duration=33)
