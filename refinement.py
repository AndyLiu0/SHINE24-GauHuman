...
from scene.gaussian_model import batch_rodrigues
...
class SMPLLoader:
    def __init__(self, model_path, regressor_path, return_keypoints=True):
        ...
        if not os.path.exists(model_path):
            print("SMPL model does not exist!")
            return
...
class InitTranslation:
    def __init__(self, solve_T=True, solve_R=False) -> None:
        """ Speficy translations and/or rotations to be refined
        """
    ...
    def __call__(self, body_model, params, cameras, keypoints):
        ...
        Args:
            ...
            keypoints (numpy.ndarray): Array of shape (#frames, 1, 25, 3) containing 3D keypoints.
        """
...
def refine(data_root, model_path):
    # Read init parameters
    body_model = SMPLLoader(model_path=f'{model_path}/SMPL_NEUTRAL.pkl', regressor_path=f'{model_path}/J_regressor_body25.npy')()['body_model']
    
    smpl_path = f'{data_root}/poses.npz'
    ...
    smpl_data = {key_mapping.get(k, k): v for k, v in smpl_data.items()}
    smpl_data['shapes'] = np.tile(smpl_data['shapes'], (smpl_data['Th'].shape[0], 1))

    keypoints_path = f'{data_root}/keypoints.npy'
    keypoints = np.load(keypoints_path)
    
    cameras_path = f'{data_root}/cameras.npz'
    cameras_data = np.load(cameras_path)
    pose = np.eye(4)
    R = pose[:3, :3]
    T = pose[:3, 3:4]
    H, W = cameras_data['height'], cameras_data['width']
    coeff = 1.2
    f = coeff * min(W, H)
    K = np.array([[f, 0, W//2], [0, f, H//2],[0, 0, 1]])
    cameras = {}
    cameras['K'] = np.tile(K, (np.shape(keypoints)[0], 1, 1))
    
    # Refinement
    ref = InitTranslation(solve_T=True, solve_R=False)
    res = ref(body_model, smpl_data, cameras, keypoints)

    # Save results
    save_path = f"{data_root}/smpl_refined"
...
if __name__ == "__main__":
    refine('/root/gauhuman/GauHuman/data/andy2', "data/easymocap")
